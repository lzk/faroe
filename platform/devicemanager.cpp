...
void DeviceManager::watchDevice()
{
    ...
    if(device){
        ...
        if(currentDeviceName.startsWith("USB")){
            ...
            device = new Device(url.toUtf8().data() ,&netIO ,(PlatformApp*)&platformApp);
        }
    }
...
{
    QJsonObject jsonobj = QJsonDocument::fromJson(obj.toUtf8()).object();
