...
#ifndef WRAPPERQIMAGE_H
#define WRAPPERQIMAGE_H
...
class WrapperQImage : public LuminanceSource
{
public:
    WrapperQImage();
    WrapperQImage(const QImage& sourceImage);
    WrapperQImage(WrapperQImage& otherInstance);
    ~WrapperQImage();

    static WrapperQImage* Factory(const QImage& image, int maxWidth=-1, int maxHeight=-1, bool smoothTransformation=false);
...
private:
    ...
    static const ubyte B_TO_GREYSCALE[256];
    static const ubyte G_TO_GREYSCALE[256];
    static const ubyte R_TO_GREYSCALE[256];
};

#endif //WRAPPERQIMAGE_H
